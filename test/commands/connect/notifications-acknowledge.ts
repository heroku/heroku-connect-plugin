...
import Acknowledge from '../../../src/commands/connect/notifications/acknowledge.js'
...
describe('connect:notifications:acknowledge', () => {
  ...
  it('acknowledges notifications successfully', async () => {
    ...
    const acknowledgeApi = nock('https://hc-virginia-qa.herokai.com/', {reqheaders: headers})
      .post('/api/v3/connections/1234/notifications/acknowledge')
      .query({page_size: 1000})
      .reply(204)

    const {stderr, stdout} = await runCommand(Acknowledge, ['--app', appName])
  ...
  it('acknowledges notifications with filters', async () => {
    ...
    const {stderr, stdout} = await runCommand(Acknowledge, [
      '--app',
      appName,
      '--after',
      '2024-01-01',
  ...
  it('handles API errors correctly', async () => {
    ...
    const acknowledgeApi = nock('https://hc-virginia-qa.herokai.com/', {reqheaders: headers})
      .post('/api/v3/connections/1234/notifications/acknowledge')
      .query({page_size: 1000})
      .reply(400, {message: 'Invalid request parameters'})
