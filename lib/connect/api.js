...
let withConnection = exports.withConnection = function (context, heroku) {
  return co(function * () {
    ...
    const resourceFlag = context.flags.resource
    const resourceFlagQueryParam = resourceFlag ? `&resource_name=${resourceFlag}` : ''
    const connectionSearchResponse = yield discoveryClient(`/connections?app=${context.app}${resourceFlagQueryParam}`)
    const connections = connectionSearchResponse.data.results
    ...
    if (connections.length === 0) {
      ...
      const matchDetailResponse = yield connectClient.get(`${match.detail_url}?deep=true`)
