...
module.exports = {
  ...
  run: cli.command(co.wrap(function* (context, heroku) {
    let redir;
    yield cli.action('authorizing salesforce', co(function* () {
    ...
    yield cli.action('waiting for authorization', co(function* () {
      yield callbackServer();
    }));

    // Necessary because simply shutting down the callback server doesn't
    // actually cause the process to exit properly.
    process.exit();
  }))
}
